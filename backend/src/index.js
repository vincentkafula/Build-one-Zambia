...
app.post(`${BASE}/data-entry/ecz-figures`, auth.requireAuth, (req, res) => { const { levelType, levelId, levelName, electionType, candidates: rawC, figures, totalVotes, totalVotesCast, rejectedBallots, enteredBy, source, notes, constituencyId, districtId, provinceId } = req.body; if (!levelType || !levelId || !electionType) return res.status(400).json({ error: 'levelType, levelId, electionType required' }); const idx = dataEntryStore.eczFigures.findIndex(f => f.levelType === levelType && f.levelId === levelId && f.electionType === electionType); const figure = { levelType, levelId, levelName: levelName || levelId, electionType, figures: figures || rawC || [], totalVotes: Number(totalVotesCast ?? totalVotes ?? 0), totalVotesCast: Number(totalVotesCast ?? totalVotes ?? 0), rejectedBallots: Number(rejectedBallots || 0), enteredBy: enteredBy || req.user?.username || '', source, notes, constituencyId, districtId, provinceId, status: 'pending', reviewedBy: null, reviewedAt: null, savedAt: new Date().toISOString(), savedBy: req.user?.username }; if (idx >= 0) dataEntryStore.eczFigures[idx] = figure; else dataEntryStore.eczFigures.push(figure); saveDataEntry(); res.json({ success: true, figure }); });
