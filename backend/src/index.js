...
app.post(`${BASE}/events`, auth.requireAuth, (req, res) => {
  const evt = { ...req.body, id: `evt-${Date.now()}`, createdAt: new Date().toISOString(), status: req.body.status || 'upcoming' };
...
app.patch(`${BASE}/events/:id`, auth.requireAuth, (req, res) => {
  ...
  eventsStore.events[idx] = { ...eventsStore.events[idx], ...req.body, updatedAt: new Date().toISOString() };
  saveEvents();
...
const memberStore = { members: kv.get('membership:members') || [] };
function saveMembers() { kv.set('membership:members', memberStore.members); }

app.post(`${BASE}/membership/register`, (req, res) => {
  const member = { ...req.body, id: `mem-${Date.now()}`, status: 'active', joinedAt: new Date().toISOString(), adoptionStatus: 'not_adopted' };
  memberStore.members.push(member);
  saveMembers();
  res.json({ success: true, member });
});

app.get(`${BASE}/membership/me`, (req, res) => {
  res.json({ member: null, eligibility: { membership: { eligible: true }, adoption: { eligible: false, reason: 'Not yet a member' } } });
});

app.get(`${BASE}/membership/members`, auth.requireAuth, (req, res) => {
  let members = [...memberStore.members];
  if (req.query.status) members = members.filter(m => m.status === req.query.status);
  const limit  = parseInt(req.query.limit  || '100', 10);
  const offset = parseInt(req.query.offset || '0',   10);
  res.json({ members: members.slice(offset, offset + limit), total: members.length });
});

app.get(`${BASE}/membership/members/:id`, auth.requireAuth, (req, res) => {
  const member = memberStore.members.find(m => m.id === req.params.id);
  if (!member) return res.status(404).json({ error: 'Member not found' });
  res.json({ member });
});

app.patch(`${BASE}/membership/members/:id`, auth.requireAuth, (req, res) => {
  const idx = memberStore.members.findIndex(m => m.id === req.params.id);
  if (idx < 0) return res.status(404).json({ error: 'Member not found' });
  memberStore.members[idx] = { ...memberStore.members[idx], ...req.body, updatedAt: new Date().toISOString() };
  saveMembers();
  res.json({ success: true, member: memberStore.members[idx] });
});

app.post(`${BASE}/membership/members/:id/grant-adoption`, auth.requireAuth, (req, res) => {
  const idx = memberStore.members.findIndex(m => m.id === req.params.id);
  if (idx < 0) return res.status(404).json({ error: 'Member not found' });
  memberStore.members[idx] = { ...memberStore.members[idx], adoptionStatus: 'adopted', adoptedAt: new Date().toISOString(), adoptedBy: req.user?.username };
  saveMembers();
  res.json({ success: true, member: memberStore.members[idx] });
});
...
app.post(`${BASE}/membership/members/:id/revoke-adoption`, auth.requireAuth, (req, res) => {
  const idx = memberStore.members.findIndex(m => m.id === req.params.id);
  if (idx < 0) return res.status(404).json({ error: 'Member not found' });
  memberStore.members[idx] = { ...memberStore.members[idx], adoptionStatus: 'not_adopted', adoptionRevokedAt: new Date().toISOString() };
  saveMembers();
  res.json({ success: true, member: memberStore.members[idx] });
});
...
app.post(`${BASE}/membership/members/:id/link-order`, auth.requireAuth, (req, res) => {
  const idx = memberStore.members.findIndex(m => m.id === req.params.id);
  if (idx < 0) return res.status(404).json({ error: 'Member not found' });
  memberStore.members[idx] = { ...memberStore.members[idx], orderId: req.body.orderId };
  saveMembers();
  res.json({ success: true, member: memberStore.members[idx] });
});

app.get(`${BASE}/membership/stats`, auth.requireAuth, (req, res) => {
  const members = memberStore.members;
  res.json({ total: members.length, active: members.filter(m => m.status === 'active').length, adopted: members.filter(m => m.adoptionStatus === 'adopted').length, byProvince: {} });
});
