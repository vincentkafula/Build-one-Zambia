...
app.patch(`${BASE}/membership/members/:id`, auth.requireAuth, auth.requireRole('admin', 'super_admin'), (req, res) => { const m = registrations.updateMember(req.params.id, req.body); if (!m) return res.status(404).json({ error: 'Not found' }); res.json({ member: m }); });
...
app.post(`${BASE}/data-entry/ecz-figures`, auth.requireAuth, (req, res) => { const { levelType, levelId, levelName, electionType, candidates: rawC, figures, totalVotes, totalVotesCast, rejectedBallots, enteredBy, source, notes } = req.body; if (!levelType || !levelId || !electionType) return res.status(400).json({ error: 'levelType, levelId, electionType required' }); const idx = dataEntryStore.eczFigures.findIndex(f => f.levelType === levelType && f.levelId === levelId && f.electionType === electionType); const figure = { levelType, levelId, levelName: levelName || levelId, electionType, figures: figures || rawC || [], totalVotes: Number(totalVotesCast ?? totalVotes ?? 0), totalVotesCast: Number(totalVotesCast ?? totalVotes ?? 0), rejectedBallots: Number(rejectedBallots || 0), enteredBy: enteredBy || req.user?.username || '', source, notes, savedAt: new Date().toISOString(), savedBy: req.user?.username }; if (idx >= 0) dataEntryStore.eczFigures[idx] = figure; else dataEntryStore.eczFigures.push(figure); saveDataEntry(); res.json({ success: true, figure }); });
app.get(`${BASE}/data-entry/ecz-figures`, auth.requireAuth, (req, res) => { let figs = [...dataEntryStore.eczFigures]; if (req.query.electionType) figs = figs.filter(f => f.electionType === req.query.electionType); if (req.query.levelType) figs = figs.filter(f => f.levelType === req.query.levelType); res.json({ figures: figs, count: figs.length }); });
