...
import { newsApi, Post, PostListItem, PostCategory } from '../../lib/api';

const CATEGORIES: (PostCategory | 'ALL')[] = ['ALL', 'NEWS', 'PRESS_RELEASE', 'CAMPAIGN_UPDATE', 'ANNOUNCEMENT', 'MEDIA'];
const CATEGORY_LABEL: Record<string, string> = {
  ALL: 'ALL', NEWS: 'NEWS', PRESS_RELEASE: 'PRESS RELEASE', CAMPAIGN_UPDATE: 'CAMPAIGN', ANNOUNCEMENT: 'ANNOUNCEMENT', MEDIA: 'MEDIA',
};
...
function imgSrc(p: PostListItem | Post) {
  return p.hasCustomImage ? newsApi.postImageUrl(p.id) : p.coverImageUrl || 'https://images.unsplash.com/photo-1495020689067-958852a7765e?w=900&h=500&fit=crop&auto=format';
}

function ArticleView({ article, onBack }: { article: Post; onBack: () => void }) {
  return (
    <div style={{ maxWidth: '820px', margin: '0 auto', padding: '64px 16px 96px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '20px', flexWrap: 'wrap' }}>
        ...
        <span style={{ display: 'flex', alignItems: 'center', gap: '5px', fontSize: '12px', color: '#4b5563' }}>
          <Calendar style={{ width: '12px', height: '12px' }} /> {formatDate(article.publishedAt || article.createdAt)}
        </span>
        <span style={{ display: 'flex', alignItems: 'center', gap: '5px', fontSize: '12px', color: '#4b5563' }}>
      ...
      <div style={{ marginBottom: '40px', overflow: 'hidden' }}>
        <img src={imgSrc(article)} alt={article.title} style={{ width: '100%', aspectRatio: '16/9', objectFit: 'cover', display: 'block' }} />
      </div>
      ...
      {/* Body is stored as one block of text; split on blank lines so
          paragraph breaks the author actually typed are preserved */}
      <div style={{ display: 'flex', flexDirection: 'column', gap: '22px' }}>
        {article.body.split(/\n\s*\n/).filter(Boolean).map((para, i) => (
          <p key={i} style={{ fontSize: '15px', lineHeight: 1.9, color: '#c0c0c0', margin: 0, whiteSpace: 'pre-wrap' }}>
            {para}
          </p>
        ))}
      </div>
      ...
      <div style={{ marginTop: '56px', paddingTop: '24px', borderTop: '1px solid #1f1f1f', display: 'flex', alignItems: 'center', gap: '8px' }}>
        ...
        <span style={{ fontSize: '12px', color: '#4b5563', fontFamily: 'Oswald, sans-serif', letterSpacing: '0.08em' }}>
          {CATEGORY_LABEL[article.category] || article.category} · {formatDate(article.publishedAt || article.createdAt)} · {article.author}
        </span>
      </div>
    </div>
  );
...
function ArticleCard({ article, onRead, featured }: { article: PostListItem; onRead: () => void; featured?: boolean }) {
    ...
    return (
      ...
      >
        <div style={{ overflow: 'hidden', maxHeight: '400px' }}>
          <img src={imgSrc(article)} alt={article.title} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block', transform: hovered ? 'scale(1.04)' : 'scale(1)', transition: 'transform 0.5s' }} />
        </div>
        <div style={{ padding: '40px 36px', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '16px', flexWrap: 'wrap' }}>
            ...
            <span style={{ backgroundColor: 'rgba(220,38,38,0.12)', color: '#dc2626', fontSize: '10px', padding: '3px 10px', fontFamily: 'Oswald, sans-serif', letterSpacing: '0.12em' }}>{CATEGORY_LABEL[article.category] || article.category}</span>
            <span style={{ fontSize: '11px', color: '#4b5563', display: 'flex', alignItems: 'center', gap: '4px' }}><Clock style={{ width: '10px', height: '10px' }} />{formatDate(article.publishedAt || article.createdAt)}</span>
          </div>
          ...
          <p style={{ fontSize: '14px', lineHeight: 1.8, color: '#6b7280', marginBottom: '24px' }}>{article.summary}</p>
  ...
  return (
    ...
    >
      <div style={{ overflow: 'hidden', height: '210px' }}>
        <img src={imgSrc(article)} alt={article.title} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block', transform: hovered ? 'scale(1.05)' : 'scale(1)', transition: 'transform 0.5s' }} />
      </div>
      <div style={{ padding: '24px', flex: 1, display: 'flex', flexDirection: 'column' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px', flexWrap: 'wrap' }}>
          <span style={{ backgroundColor: 'rgba(220,38,38,0.12)', color: '#dc2626', fontSize: '10px', padding: '3px 9px', fontFamily: 'Oswald, sans-serif', letterSpacing: '0.1em' }}>{CATEGORY_LABEL[article.category] || article.category}</span>
          <span style={{ fontSize: '11px', color: '#4b5563', display: 'flex', alignItems: 'center', gap: '4px' }}><Clock style={{ width: '10px', height: '10px' }} />{formatDate(article.publishedAt || article.createdAt)}</span>
        </div>
        ...
        <p style={{ fontSize: '13px', lineHeight: 1.75, color: '#6b7280', marginBottom: '20px' }}>{article.summary}</p>
...
export function NewsPage() {
  ...
  const [openArticle, setOpenArticle] = useState<Post | null>(null);
  ...
  useEffect(() => {
    setLoading(true);
    newsApi.listPosts({ status: 'published', limit: 100 })
      .then(res => setPosts(res.posts))
      .catch(() => setLoadError('Could not load news right now. Please try again shortly.'))
      .finally(() => setLoading(false));
  }, []);

  useEffect(() => {
    if (!openArticleId) { setOpenArticle(null); return; }
    newsApi.getPost(openArticleId).then(res => setOpenArticle(res.post)).catch(() => setOpenArticleId(null));
  }, [openArticleId]);
        ...
        <div style={{ backgroundColor: '#007A30', borderBottom: '1px solid #005020', padding: '16px' }}>
          <div style={{ maxWidth: '820px', margin: '0 auto' }}>
            <p style={{ fontSize: '11px', letterSpacing: '0.18em', color: '#4b5563', fontFamily: 'Oswald, sans-serif', margin: 0 }}>
              NEWS {openArticle ? `/ ${CATEGORY_LABEL[openArticle.category] || openArticle.category}` : ''}
            </p>
          </div>
        </div>
        {openArticle ? (
          <ArticleView article={openArticle} onBack={() => setOpenArticleId(null)} />
        ) : (
          <div style={{ padding: '96px 16px', textAlign: 'center', color: '#9ca3af' }}>Loading…</div>
        )}
      </div>
    );
  }
        ...
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          ...
          {loading && (
            <div style={{ textAlign: 'center', padding: '80px 16px', color: '#4b5563' }}>
              <p style={{ fontFamily: 'Oswald, sans-serif', fontSize: '1.2rem', letterSpacing: '0.06em' }}>Loading news…</p>
            </div>
          )}

          {!loading && loadError && (
            <div style={{ textAlign: 'center', padding: '80px 16px', color: '#dc2626' }}>
              <p style={{ fontFamily: 'Oswald, sans-serif', fontSize: '1.1rem', letterSpacing: '0.06em' }}>{loadError}</p>
            </div>
          )}
          ...
          {!loading && !loadError && featured && (
            <div style={{ marginBottom: '32px' }}>
              <ArticleCard article={featured} onRead={() => setOpenArticleId(featured.id)} featured />
            </div>
          )}
          ...
          {!loading && !loadError && rest.length > 0 && (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '24px' }}>
              {rest.map(article => (
                <ArticleCard key={article.id} article={article} onRead={() => setOpenArticleId(article.id)} />
              ))}
            </div>
          )}
        </div>
