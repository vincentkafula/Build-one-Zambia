...
import { Building, Lock, ArrowRight, ArrowLeft, CheckCircle, Eye, EyeOff, Copy, Loader2 } from 'lucide-react';
...
export default function ChamberRegistration() {
  ...
  const { register, handleSubmit, watch } = useForm<ChamberFormData>();

  const onStep1Submit = (data: ChamberFormData) => {
    if (!data.chamberName || !data.contactPerson || !data.phone || !data.email) {
  ...
  const onStep2Submit = async () => {
    ...
    setSubmitError('');
    try {
      const res = await registrationApi.submitChamber({
        ...
        phone: data.phone,
        email: data.email,
        memberBusinesses: data.memberBusinesses,
        ...
        <div className="w-full max-w-2xl relative text-center">
          <div className="rounded-2xl p-12" style={{ backgroundColor: '#fff', boxShadow: '0 8px 32px rgba(0,0,0,0.1)' }}>
            ...
            <p className="mb-8 text-lg" style={{ color: '#6b7280' }}>
              Your chamber of commerce has been submitted to Build One Zambia!
            </p>
            ...
            <div className="p-6 rounded-lg mb-6 text-left" style={{ backgroundColor: `${ACCENT_COLOR}08`, border: `1px solid ${ACCENT_COLOR}30` }}>
              ...
              <ul className="text-sm space-y-1 list-disc list-inside" style={{ color: '#4b5563' }}>
                <li>An admin will review your chamber's registration</li>
            ...
            {activatedInfo ? (
              ...
              <button
                onClick={async () => {
                  try {
                    const res = await registrationApi.getChamberCredentials(registrationId);
      ...
      <div className="max-w-2xl mx-auto relative">
        <div className="text-center mb-8">
          ...
          <p className="text-sm" style={{ color: '#6b7280' }}>
            Register your ward or district chamber of commerce with Build One Zambia
          </p>
        </div>
        ...
        <div className="mb-8">
          <div className="flex items-center justify-center mb-3">
            {[
              { num: 1, label: 'Chamber Details', icon: Building },
            ...
            {currentStep === 1 && (
              <form onSubmit={handleSubmit(onStep1Submit)}>
                ...
                <div className="space-y-4">
                  <div>
                    <label className="block text-sm font-medium mb-1.5" style={{ color: '#374151' }}>Chamber Name *</label>
                    <input {...register('chamberName', { required: true })} placeholder="e.g. Kabwata Ward Chamber of Commerce"
                      className="w-full px-4 py-3 rounded-lg" style={{ border: '1px solid #d1d5db' }} />
                  </div>

                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium mb-1.5" style={{ color: '#374151' }}>Ward</label>
                      <input {...register('ward')} placeholder="e.g. Kabwata Ward"
                        className="w-full px-4 py-3 rounded-lg" style={{ border: '1px solid #d1d5db' }} />
                    </div>
                    <div>
                      <label className="block text-sm font-medium mb-1.5" style={{ color: '#374151' }}>District</label>
                      <input {...register('district')} placeholder="e.g. Lusaka"
                        className="w-full px-4 py-3 rounded-lg" style={{ border: '1px solid #d1d5db' }} />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <input {...register('contactTitle')} placeholder="e.g. Chairperson"
                        className="w-full px-4 py-3 rounded-lg" style={{ border: '1px solid #d1d5db' }} />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input {...register('phone', { required: true })} placeholder="+260 97 000 0000"
                        className="w-full px-4 py-3 rounded-lg" style={{ border: '1px solid #d1d5db' }} />
                    </div>
                    <div>
                      <label className="block text-sm font-medium mb-1.5" style={{ color: '#374151' }}>Email *</label>
                      <input type="email" {...register('email', { required: true })} placeholder="chamber@example.com"
                        className="w-full px-4 py-3 rounded-lg" style={{ border: '1px solid #d1d5db' }} />
                    </div>
                  </div>

                  <div>
                    <label className="block text-sm font-medium mb-1.5" style={{ color: '#374151' }}>Approx. Number of Member Businesses</label>
                    <input {...register('memberBusinesses')} placeholder="e.g. 45"
                      className="w-full px-4 py-3 rounded-lg" style={{ border: '1px solid #d1d5db' }} />
                  </div>

                  <div>
                    <label className="block text-sm font-medium mb-1.5" style={{ color: '#374151' }}>Focus Areas</label>
                    <input {...register('focusAreas')} placeholder="e.g. Retail, agro-processing, transport"
                      className="w-full px-4 py-3 rounded-lg" style={{ border: '1px solid #d1d5db' }} />
                  </div>
                </div>
            ...
            {currentStep === 2 && (
              <div>
                ...
                <p className="text-sm text-gray-500 mb-6">
                  Choose the password and PIN your chamber's contact person will use to log in and manage
                  your chamber's profile. The account is created now but stays inactive until an admin
                ...
                <div className="flex gap-4">
                  ...
                  <button
                    ...
                    style={{ backgroundColor: ACCENT_COLOR, fontFamily: 'Oswald, sans-serif', letterSpacing: '0.08em', fontSize: '14px' }}
                  >
                    {submitting ? <><Loader2 className="w-4 h-4 animate-spin" /> SUBMITTING…</> : <>SUBMIT CHAMBER <ArrowRight className="w-4 h-4" /></>}
                  </button>
                </div>
              </div>
            )}
